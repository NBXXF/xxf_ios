...
- [ ] 每个方法的 `///` 涵盖到:**具体功能**、**使用限制/前置条件**、**线程限制**、**边界/幂等**、**副作用**(不相关可省,相关的不能漏)?
- [ ] 每个参数用 `- Parameter` 列出含义/单位/取值范围/是否允许 nil/回调线程?返回值 `- Returns:` 说明特殊情况?抛出 `- Throws:` 说明什么条件抛什么错?
- [ ] 每个成员字段(let/var/computed)都有 `///` 说明作用,对外字段标出语义边界?
