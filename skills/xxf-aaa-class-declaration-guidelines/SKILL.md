...
1. 先识别当前 VC/VM 的属性和方法职责。
2. 在不改变业务行为前提下重排顺序并补 MARK。
3. 为缺失注释的字段和方法补齐用途说明与关键副作用说明。
4. 合并重复或语义重叠的分区名（如 UIInit/UISetup 统一为 Setup UI）。
5. 若单文件持续膨胀，提出 extension 拆分并执行最小可行拆分。
