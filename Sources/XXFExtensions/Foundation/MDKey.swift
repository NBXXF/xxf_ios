...
enum MDKey {
    ...
    static let attributeChangeDate = kMDItemAttributeChangeDate as String
    
    /// 内容的目标受众 (CFArray[CFString])
    /// 例如: ["开发者", "设计师", "教育工作者"]
    /// 用法示例: value(forAttribute: MDKey.audiences) as? [String]
    static let audiences = kMDItemAudiences as String
    ...
    static let authorAddresses = kMDItemAuthorAddresses as String
    ...
    static let country = kMDItemCountry as String
    ...
    static let dateAdded = kMDItemDateAdded as String
    ...
    static let displayName = kMDItemDisplayName as String
    
    /// 文件下载时间 (CFDate)
    /// 例如: 从互联网下载文件的时间
    /// 用法示例: value(forAttribute: MDKey.downloadedDate) as? Date
    static let downloadedDate = kMDItemDownloadedDate as String
    
    /// 任务截止日期 (CFDate)
    /// 例如: 项目提交截止日
    /// 用法示例: value(forAttribute: MDKey.dueDate) as? Date
    static let dueDate = kMDItemDueDate as String
    ...
    static let editors = kMDItemEditors as String
    ...
    static let identifier = kMDItemIdentifier as String
    
    /// 内容摘要信息 (CFString)
    /// 例如: "本报告包含机密财务数据"
    /// 用法示例: value(forAttribute: MDKey.information) as? String ?? ""
    static let information = kMDItemInformation as String
    ...
    static let instantMessageAddresses = kMDItemInstantMessageAddresses as String
    
    /// 使用说明 (CFString)
    /// 例如: "打印时使用A4纸，彩色模式"
    /// 用法示例: value(forAttribute: MDKey.instructions) as? String ?? ""
    static let instructions = kMDItemInstructions as String
    
    /// 关键词列表 (CFArray[CFString])
    /// 例如: ["财务", "2023", "季度报告"]
    /// 用法示例: value(forAttribute: MDKey.keywords) as? [String]
    static let keywords = kMDItemKeywords as String
    
    /// 文件类型描述 (CFString)
    /// 例如: "PDF 文档", "JPEG 图像"
    /// 用法示例: value(forAttribute: MDKey.kind) as? String ?? ""
    static let kind = kMDItemKind as String
    
    /// 内容语言列表 (CFArray[CFString])
    /// 例如: ["zh-Hans", "en"]
    /// 用法示例: value(forAttribute: MDKey.languages) as? [String]
    static let languages = kMDItemLanguages as String
    
    /// 文件最后使用时间 (CFDate)
    /// 例如: 文档最近打开时间
    /// 用法示例: value(forAttribute: MDKey.lastUsedDate) as? Date
    static let lastUsedDate = kMDItemLastUsedDate as String
    ...
    static let organizations = kMDItemOrganizations as String
    ...
    static let phoneNumbers = kMDItemPhoneNumbers as String
    
    /// 所属项目列表 (CFArray[CFString])
    /// 例如: ["火星计划", "年度财报"]
    /// 用法示例: value(forAttribute: MDKey.projects) as? [String]
    static let projects = kMDItemProjects as String
    ...
    static let recipients = kMDItemRecipients as String
    ...
    static let recipientAddresses = kMDItemRecipientAddresses as String
    ...
    static let recipientEmailAddresses = kMDItemRecipientEmailAddresses as String
    
    /// 权限声明 (CFString)
    /// 例如: "仅限内部使用", "知识共享署名许可"
    /// 用法示例: value(forAttribute: MDKey.rights) as? String ?? ""
    static let rights = kMDItemRights as String
    
    /// 安全加密方法 (CFString)
    /// 例如: "密码保护", "AES-256"
    /// 用法示例: value(forAttribute: MDKey.securityMethod) as? String ?? ""
    static let securityMethod = kMDItemSecurityMethod as String
    
    /// 用户星级评分 (CFNumber)
    /// 范围: 0-5 (例如: 4.5)
    /// 用法示例: value(forAttribute: MDKey.starRating) as? Double ?? 0.0
    static let starRating = kMDItemStarRating as String
    
    /// 州/省名称 (CFString)
    /// 例如: "加利福尼亚州", "东京都"
    /// 用法示例: value(forAttribute: MDKey.stateOrProvince) as? String ?? ""
    static let stateOrProvince = kMDItemStateOrProvince as String
    
    /// 内容主题 (CFString)
    /// 例如: "可持续发展", "人工智能伦理"
    /// 用法示例: value(forAttribute: MDKey.subject) as? String ?? ""
    static let subject = kMDItemSubject as String
    ...
    static let theme = kMDItemTheme as String
    ...
    static let url = kMDItemURL as String
    ...
    static let whereFroms = kMDItemWhereFroms as String
    ...
    static let applicationCategories = kMDItemApplicationCategories as String
    
    /// 应用Bundle标识符 (CFString)
    /// 例如: "com.apple.Keynote"
    /// 用法示例: value(forAttribute: MDKey.cfBundleIdentifier) as? String ?? ""
    static let cfBundleIdentifier = kMDItemCFBundleIdentifier as String
    
    /// 可执行文件支持的架构 (CFArray[CFString])
    /// 例如: ["x86_64", "arm64"]
    /// 用法示例: value(forAttribute: MDKey.executableArchitectures) as? [String]
    static let executableArchitectures = kMDItemExecutableArchitectures as String
    
    /// 可执行平台要求 (CFString)
    /// 例如: "macOS"
    /// 用法示例: value(forAttribute: MDKey.executablePlatform) as? String ?? ""
    static let executablePlatform = kMDItemExecutablePlatform as String
    
    /// 是否由应用管理 (CFBoolean)
    /// 例如: iCloud 托管文件
    /// 用法示例: value(forAttribute: MDKey.isApplicationManaged) as? Bool ?? false
    static let isApplicationManaged = kMDItemIsApplicationManaged as String
    
    /// 是否可能为垃圾内容 (CFBoolean)
    /// 主要应用于邮件判断
    /// 用法示例: value(forAttribute: MDKey.isLikelyJunk) as? Bool ?? false
    static let isLikelyJunk = kMDItemIsLikelyJunk as String
    ...
    static let appleLoopDescriptors = kMDItemAppleLoopDescriptors as String
    ...
    static let htmlContent = kMDItemHTMLContent as String
    
    /// 总页数 (CFNumber)
    /// 例如: 24
    /// 用法示例: value(forAttribute: MDKey.numberOfPages) as? Int ?? 0
    static let numberOfPages = kMDItemNumberOfPages as String
    
    /// 页面高度（点） (CFNumber)
    /// 1点 = 1/72英寸
    /// 用法示例: value(forAttribute: MDKey.pageHeight) as? Double ?? 0.0
    static let pageHeight = kMDItemPageHeight as String
    
    /// 页面宽度（点） (CFNumber)
    /// 1点 = 1/72英寸
    /// 用法示例: value(forAttribute: MDKey.pageWidth) as? Double ?? 0.0
    static let pageWidth = kMDItemPageWidth as String
}
