...
public final class Tracker: @unchecked Sendable {
    ...
    private let isolationQueue = DispatchQueue(
        label: "com.xxf.tracker.isolation",
    ...
    public func track(data: Any, extra: [AnyHashable: Any] = [:]) {
        // 快速 snapshot 共享状态
        let (converters, channels, handleCallback) = isolationQueue.sync { () -> (
            [TrackerConverter], [ChanelTracker], TrackerHandleCallback?
        ) in
            return (self._trackerConverters,
                    self._chanelBatchTrackers,
                    self._trackerHandleCallback)
        }
        ...
        for chanel in channels {
            var tempExtra = extraMutable
            var convertResult: String?

            if converters.first(where: {
                let c = $0.convert(data: data, extra: &tempExtra, chanel: chanel)
                if let s = c {
                    convertResult = s
                    return true
                }
                ...
                for (k, v) in tempExtra {
                    merged[k] = v
                }
                chanel.onTracking(data: str, extra: merged)
            }
        }
    }
