...
public extension UICollectionView {
  ...
  final func dequeueReusableCell<T: UICollectionViewCell>(for indexPath: IndexPath, cellType: T.Type = T.self) -> T
    where T: Reusable {
        ...
        // 自动注册 class 或 nib
        if let nibType = T.self as? NibLoadable.Type {
            self.register(nibType.nib, forCellWithReuseIdentifier: reuseID)
        } else {
            self.register(T.self, forCellWithReuseIdentifier: reuseID)
        }
  ...
  final func dequeueReusableSupplementaryView<T: UICollectionReusableView>
    ...
    where T: Reusable {
        let reuseID = viewType.reuseIdentifier

        // 自动注册 class 或 nib
        if let nibType = T.self as? NibLoadable.Type {
            self.register(nibType.nib, forSupplementaryViewOfKind: elementKind, withReuseIdentifier: reuseID)
        } else {
            self.register(T.self, forSupplementaryViewOfKind: elementKind, withReuseIdentifier: reuseID)
        }
