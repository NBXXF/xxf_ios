...
public extension UITableView {
  ...
  final func dequeueReusableCell<T: UITableViewCell>(for indexPath: IndexPath, cellType: T.Type = T.self) -> T
    where T: Reusable {
        ...
        if let nibType = T.self as? NibLoadable.Type {
            self.register(nibType.nib, forCellReuseIdentifier: reuseID)
        } else {
  ...
  final func dequeueReusableHeaderFooterView<T: UITableViewHeaderFooterView>(_ viewType: T.Type = T.self) -> T?
    where T: Reusable {
        ...
        // 自动注册
        if let nibType = T.self as? NibLoadable.Type {
            self.register(nibType.nib, forHeaderFooterViewReuseIdentifier: reuseID)
        } else {
            self.register(T.self, forHeaderFooterViewReuseIdentifier: reuseID)
        }
