...
public extension MoyaProvider where Target: Sendable {
    private func getCallbackQueue() -> DispatchQueue {
        if let superMoyaProvider = self as? HttpMoyaProvider {
            return superMoyaProvider.publicCallbackQueue ?? .main
        }
        return .main
    }
                       ...
                       completion: @escaping DataStreamRequest.Handler<Data, Never>) -> DataStreamRequest
    {
        ...
        streamRequest.responseStream(on: callbackQueue ?? getCallbackQueue(), stream: completion)
        return streamRequest
    }
    ...
    {
        let streamRequest = requestStream(callbackQueue: callbackQueue, target)
        streamRequest.responseStreamString(on: callbackQueue ?? getCallbackQueue(), stream: completion)
        return streamRequest
    }
    ...
    {
        let streamRequest = requestStream(callbackQueue: callbackQueue, target)
        streamRequest.responseStream(using: serializer, on: callbackQueue ?? getCallbackQueue(), stream: completion)
        return streamRequest
    }
    ...
    func requestStreamDecodable<T: Decodable & Sendable>(
        ...
        let streamRequest = requestStream(callbackQueue: callbackQueue, target)
        streamRequest.responseStreamDecodable(of: type, on: callbackQueue ?? getCallbackQueue(), using: decoder, preprocessor: preprocessor, stream: completion)
        return streamRequest
    }
}
