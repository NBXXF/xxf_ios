...
public enum Json {
    ...
    private nonisolated(unsafe) static var _defaultDecoder: JSONDecoder = .init()
    private nonisolated(unsafe) static var _defaultEncoder: JSONEncoder = .init()
    private static let lockQueue = DispatchQueue(label: "com.xxf.json.LockQueue")

    // 配置版本号，用于检测缓存是否过期
    private nonisolated(unsafe) static var configVersion: Int = 0

    public static var defaultDecoder: JSONDecoder {
        get { lockQueue.sync { _defaultDecoder } }
        set {
            lockQueue.sync {
    ...
    public static var defaultEncoder: JSONEncoder {
        get { lockQueue.sync { _defaultEncoder } }
        set {
            lockQueue.sync {
