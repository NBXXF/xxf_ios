...
import Foundation
    ...
    public struct NSMetadataQueryResult: NSMetadataProtocol, NSMetadataOperationProtocol {
        ...
        public func value<T>(forMetadataItem key: String) -> T? {
            let rawValue = metadataDict[key]

            // 1. 直接尝试类型转换
            if let typed = rawValue as? T {
                return typed
            }
            ...
            if let str = rawValue as? String {
                switch T.self {
                    case is Int.Type:
                        return Self.parseInt(str) as? T

                    case is Int?.Type:
                        return Self.parseOptionalInt(str) as? T

                    case is Double.Type:
                        return Self.parseDouble(str) as? T

                    case is Double?.Type:
                        return Self.parseOptionalDouble(str) as? T

                    case is Bool.Type, is Bool?.Type:
                        return Self.parseBool(str) as? T

                    case is Date.Type, is Date?.Type:
                        return Self.parseOptionalDate(str) as? T

                    case is URL.Type, is URL?.Type:
                        return URL(string: str) as? T

                    case is [String].Type, is [String]?.Type:
                        return Self.parseStringArray(str) as? T

                    case is [Int].Type, is [Int]?.Type:
                        return Self.parseIntArray(str) as? T

                    case is [Double].Type, is [Double]?.Type:
                        return Self.parseDoubleArray(str) as? T

                    case is Data.Type, is Data?.Type:
                        // 试图从Base64字符串解析Data
                        return Data(base64Encoded: str) as? T
            ...
            if T.self == [Int].self || T.self == [Int]?.self {
                if let arr = rawValue as? [Int] {
                    return arr as? T
                } else if let arr = rawValue as? [NSNumber] {
                    return arr.map { $0.intValue } as? T
                }
            }

            // [Double]
            if T.self == [Double].self || T.self == [Double]?.self {
                if let arr = rawValue as? [Double] {
                    return arr as? T
                } else if let arr = rawValue as? [NSNumber] {
                    return arr.map { $0.doubleValue } as? T
                }
            }
            ...
            // CGFloat
            if T.self == CGFloat.self || T.self == CGFloat?.self {
                if let dbl = rawValue as? Double {
                    return CGFloat(dbl) as? T
                }
            }

            // UUID
            if T.self == UUID.self || T.self == UUID?.self {
                if let str = rawValue as? String {
                    return UUID(uuidString: str) as? T
                }
            }
        ...
        public var mdItemUrl: URL? { value(forMetadataItem: MDKey.url) }
        ...
        public var mdItemFSInvisible: Bool { value(forMetadataItem: MDKey.fsInvisible).orFalse }
        ...
        public var mdItemFSName: String { value(forMetadataItem: MDKey.fsName).orEmpty }
        ...
        public var mdItemFSPath: String { value(forMetadataItem: MDKey.fsPath).orEmpty }

        /// 文件大小 (格式: Int) - 文件大小(以字节为单位)
        public var mdItemFSSize: Int64? { value(forMetadataItem: MDKey.fsSize) }
    ...
    extension NSMetadataQueryResult {
        ...
        static func parseDouble(_ string: String?) -> Double {
            guard let cleaned = cleanString(string) else { return 0.0 }
            return Double(cleaned) ?? 0.0
        }

        static func parseOptionalDouble(_ string: String?) -> Double? {
            guard let cleaned = cleanString(string) else { return nil }
            return Double(cleaned)
        }

        static func parseInt(_ string: String?) -> Int {
            guard let cleaned = cleanString(string) else { return 0 }
            return Int(cleaned) ?? 0
        }

        static func parseOptionalInt(_ string: String?) -> Int? {
            guard let cleaned = cleanString(string) else { return nil }
            return Int(cleaned)
        }

        static func parseBool(_ string: String?) -> Bool {
            guard let s = cleanString(string)?.lowercased() else { return false }
            if let intValue = Int(s) {
                return intValue != 0
            }
            return ["true", "yes", "on", "y"].contains(s)
        }

        static func parseOptionalDate(_ string: String?) -> Date? {
            guard let s = cleanString(string) else { return nil }
        ...
        static func parseStringArray(_ string: String?) -> [String]? {
            guard let s = cleanString(string), !s.isEmpty else { return nil }
            if s.hasPrefix("[") {
                // JSON数组字符串
        ...
        static func parseIntArray(_ string: String?) -> [Int]? {
            guard let s = cleanString(string), !s.isEmpty else { return nil }
            if s.hasPrefix("[") {
                if let data = s.data(using: .utf8),
