...
extension URL: NSMetadataProtocol, NSMetadataOperationProtocol {
    ...
    public var mdItem: MDItem? {
        ...
        // 只有是文件URL时创建
        guard let itemRef = MDItem.create(fromPath: path) else {
            return nil
        }
    ...
    public var mdItemUrl: URL? { mdItem?.mdItemUrl }
    ...
    public var mdItemFSInvisible: Bool { mdItem?.mdItemFSInvisible ?? false }
    ...
    public var mdItemFSName: String { mdItem?.mdItemFSName ?? String.empty }
    ...
    public var mdItemFSPath: String { mdItem?.mdItemFSPath ?? String.empty }

    /// 文件大小 (格式: Int?) - 文件大小(以字节为单位)
    public var mdItemFSSize: Int64? { mdItem?.mdItemFSSize }
