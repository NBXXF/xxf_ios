...
public extension Process {
    ...
    /// 检查进程是否存在
    static func isProcessAlive(processName: String) -> Bool {
        let pgrepTask = Process()
        pgrepTask.launchPath = "/usr/bin/pgrep"
        pgrepTask.arguments = ["-x", processName] // 精确匹配进程名
        pgrepTask.launch()
        pgrepTask.waitUntilExit()

        // pgrep返回0表示找到匹配的进程，非0表示未找到
        return pgrepTask.terminationStatus == 0
    }
