...
public extension PrimitiveSequence where Trait == SingleTrait {
    /// 测量当前步骤的耗时
    /// - Parameter onElapsed: (从订阅到 onSuccess/onError 的耗时, 从订阅到结束的耗时)
    /// - Returns: 原 Single
    func measureStepTime(_ onElapsed: @escaping (_ totalElapsed: TimeUnit) -> Void) -> Single<Element> {
        let sw = Stopwatch()
        return self.do(
            onSuccess: { _ in
    ...
    /// - Parameter onElapsed: (从订阅到 onSuccess/onError 的耗时, 从订阅到结束的耗时)
    /// - Returns: 原 Single
    func measureChainElapsed(_ onElapsed: @escaping (_ totalElapsed: TimeUnit) -> Void) -> Single<Element> {
        return Single.deferred { self }.measureStepTime(onElapsed)
    }
