...
public enum Schedulers {
    ...
    private static let _computation = ConcurrentDispatchQueueScheduler(qos: .userInitiated)
    ...
    private static let _single = SerialDispatchQueueScheduler(internalSerialQueueName: "rx.single")

    /// 后台调度器（最低优先级）
    /// 适合日志、缓存清理等低优先级任务
    private static let _background = ConcurrentDispatchQueueScheduler(qos: .background)
    ...
    static func background() -> SchedulerType {
        return _background
    }
